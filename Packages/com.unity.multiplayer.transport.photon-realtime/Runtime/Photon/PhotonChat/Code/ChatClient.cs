...
namespace Photon.Chat
{
    ...
    public class ChatClient : IPhotonPeerListener
    {
        ...
        private void HandleSubscribeEvent(EventData eventData)
        {
            ...
            object channels;
            if (eventData.Parameters.TryGetValue(ChatParameterCode.UniqueRoomId, out channels) == true)
            {
                channelsIDs = (int[])channels;
            }
            ...
            if (eventData.Parameters.TryGetValue(ChatParameterCode.Properties, out temp))
            {
                ...
                if (channelsInResponse.Length == 1)
                {
                    if (results[0])
                    {
                        ...
                        if (channelsIDs != null && channelsIDs.Length > 0)
                        {
                            channel.ChannelID = channelsIDs[0];
                        }
                    }
            ...
            for (int i = 0; i < channelsInResponse.Length; i++)
            {
                if (results[i])
                {
                    ...
                    if (channelsIDs != null && channelsIDs.Length > i)
                    {
                        channel.ChannelID = channelsIDs[i];
                    }
                }
            }
            this.listener.OnSubscribed(channelsInResponse, results);
